...
class AlphaPulseWindow(QMainWindow):
    ...
    def _control_panel(self) -> QWidget:
        ...
        strat_card = QLabel(
            ...
            f"AlphaPulse — Hawkes 자기자극 클러스터에 점프바 즉시 진입 + "
            f"Conviction-Power Kelly 사이징.</span><br><br>"
            f"<b>지표</b> · ATR({sp.atr_n}) × {sp.chandelier_mult:.1f}<br>"
            f"<b>점프 검출</b> · LM 임계 {sp.lm_threshold:.1f}, "
            f"screener 픽이면 inside_band 스킵<br>"
            f"<b>v2.2 cascade-test 진입</b> · 스크리너 픽 시 "
            f"Donchian 대신 anchor 대비 continuation "
            f"(Aronson 2007 §IV / Aït-Sahalia-Jacod 2009)<br>"
            f"<b>v3 P1 적응형 chandelier</b> · base × (1 + "
            f"{sp.chandelier_width_boost:.1f}·exp(−t/{sp.chandelier_decay_tau}h)) "
            f"(Aït-Sahalia-CD-Laeven 2014)<br>"
            f"<b>v3 P3 Faber pyramiding</b> · max {sp.max_pyramid_legs} "
            f"legs, in-profit only (Faber 2007 §IV; risk/N "
            f"MTZ 2011)<br>"
            f"<b>v3 A2 leader-follower 부스트</b> · "
            f"BTC/ETH 같은 방향 점프 {sp.leader_anchor_decay_bars}h 안 → "
            f"|L|×{sp.leader_anchor_boost:.2f} "
            f"(Aït-Sahalia-CD-Laeven 2014 §3 mutually exciting)<br>"
            f"<b>v3 A1 multi-timeframe LM</b> · scales="
            f"{(1,) + sp.multi_scale_lm_factors}, exit chandelier "
            f"= 3·√scale·ATR (Lee-Mykland 2008 §2.4 / Bandy 2014 §5)<br>"
            f"<b>v2.1 다중-horizon TSM</b> · "
            f"{'/'.join(f'{lb//24}d' for lb in sp.tsm_majority_lookbacks)} "
            f"≥{sp.tsm_majority_min_agree}/3 다수결 "
            f"(Han-Zhou-Zhu 2016) + 거래량 z ≥ {sp.volume_z_threshold:.1f} "
            f"(Easley-LdP-O'Hara 2012)<br>"
            f"<b>리스크</b> · CVaR α={sp.cvar_alpha:.2f}, "
            f"floor={sp.cvar_floor:.2f}<br>"
            f"<b>사이징</b> · risk_base={sp.risk_per_trade:.2%}, "
            f"<b>amp = conf<sup>{sp.confidence_exponent:.0f}</sup></b> "
            f"(quadratic)<br>"
            f"&nbsp;&nbsp;&nbsp;<span style='color:{SUBTEXT}'>약신호: 0.02% 손실 / 강신호: 2-5%</span><br>"
            f"<b>레버리지</b> · max {sp.leverage_cap:.0f}× "
            f"(신호×stop tightness 자동 산정)<br>"
            f"<b>OOS</b> · {SETTINGS.live_oos_bars} bars "
