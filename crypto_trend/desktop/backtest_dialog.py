...
class BacktestDialog(QDialog):
    ...
    def _build_form_panel(self) -> QWidget:
        ...
        strat_card = QLabel(
            ...
            f"<b>v2.2 cascade-test 진입</b><br>"
            f"&nbsp;&nbsp;스크리너 픽 시 Donchian 대신 anchor 대비 continuation "
            f"<span style='color:{SUBTEXT}'>(Aronson 2007 §IV; "
            f"Aït-Sahalia &amp; Jacod 2009 §3 — LM 검정이 breakout 휴리스틱을 "
            f"점근적으로 dominate)</span><br>"
            f"<b>v3 P1 적응형 chandelier</b><br>"
            f"&nbsp;&nbsp;mult = base × (1 + {sp.chandelier_width_boost:.1f} · "
            f"exp(−t / {sp.chandelier_decay_tau}h)) "
            f"<span style='color:{SUBTEXT}'>(Aït-Sahalia-CD-Laeven 2014 "
            f"Hawkes decay)</span><br>"
            f"<b>v3 P3 Faber pyramiding</b><br>"
            f"&nbsp;&nbsp;max {sp.max_pyramid_legs} legs, only on winners, "
            f"per-leg risk = risk_per_trade / max_legs "
            f"<span style='color:{SUBTEXT}'>(Faber 2007 §IV; "
            f"MacLean-Thorp-Ziemba 2011 §3 partial Kelly)</span><br>"
            f"<b>v3 A2 leader-follower 부스트</b><br>"
            f"&nbsp;&nbsp;BTC/ETH 같은 방향 점프 {sp.leader_anchor_decay_bars}h 안 → "
            f"|L| × {sp.leader_anchor_boost:.2f} "
            f"<span style='color:{SUBTEXT}'>(Aït-Sahalia-Cacho-Diaz-Laeven "
            f"2014 §3 mutually-exciting)</span><br>"
            f"<b>v3 A1 multi-timeframe LM</b><br>"
            f"&nbsp;&nbsp;scales = {(1,) + sp.multi_scale_lm_factors}, "
            f"chandelier = 3·√scale·ATR "
            f"<span style='color:{SUBTEXT}'>(Lee-Mykland 2008 §2.4 scale "
            f"invariance; Bandy 2014 §5 vol-scaling)</span><br>"
            f"<b>v2.1 진입 필터 (counter-trend / noise 차단)</b><br>"
            f"&nbsp;&nbsp;TSM 다중-horizon 다수결 "
            f"= {'/'.join(f'{lb//24}d' for lb in sp.tsm_majority_lookbacks)} "
            f"≥ {sp.tsm_majority_min_agree}/3 "
            f"<span style='color:{SUBTEXT}'>(Han-Zhou-Zhu 2016, AMP 2013)</span><br>"
